...
public partial class MainWindowViewModel : ViewModelBase
{
 ...
 // Also hook MainWindow.Closing to clear LEDs before teardown
 ...
 {
 Console.WriteLine($"Failed to hook app Exit event: {ex.Message}");
 }
 ...
 {
 // For other devices, still ensure LEDs reflect current state after connect
 RestoreAllLedStates();
 }
 }
 else
 ...
 {
 // Best effort to turn off LEDs when we disconnect mid-session
 TurnOffAllLeds();
 ...
 private void OnMidiControlChange(object? sender, MidiControlChangeEventArgs e)
{
 ...
 // Convert0-16383 to0-127 for display
 ...
 private const int FaderDebounceMs =30; // Batch audio updates every30ms
 ...
 // Update audio activity indicator with500ms delayed off
 ...
 // Restore all LED states based on current states
 RestoreAllLedStates();
 }

 /// <summary>
 /// Restores all LED states (mute, solo, select/audio activity, transport, record)
 /// </summary>
 ...
 // Restore select LED (audio activity indicator)
 SendSelectLedFeedback(channel, channel.IsAudioActive);
 }

 // Transport LEDs - reflect current play/pause state; ensure next/prev are off
 UpdatePlayPauseLeds();
 SetTransportLed(MidiControlType.TransportNext, false);
 SetTransportLed(MidiControlType.TransportPrevious, false);

 // Record LEDs - ensure only default devices are lit
 TurnOffAllRecordLeds();
 InitializeDefaultDeviceRecordLeds();
 }
 ...
 _midiService.SendControlChange(ch,120,0);
 ...
 {
 _midiService.SendNoteOn(mapping.Channel, mapping.ControlNumber,0);
 _midiService.SendNoteOff(mapping.Channel, mapping.ControlNumber);
 _midiService.SendControlChange(mapping.Channel, mapping.ControlNumber,0);
 }
 }

 // Special case: M-Vave SMC/SINCO3x8 matrix on channel0 (notes0x00-0x17)
 var name = SelectedMidiDevice?.Name ?? string.Empty;
 if (name.Contains("SMC", StringComparison.OrdinalIgnoreCase) ||
 ...
 /// <summary>
 /// Ensures all Record LEDs are off for channels that have a Record mapping
 /// </summary>
 private void TurnOffAllRecordLeds()
 {
 foreach (var ch in Channels)
 {
 SendRecordLedFeedback(ch, false);
 }
 }
 ...
 // Row1: Notes0x00-0x07 (top row buttons)
 // Row2: Notes0x08-0x0F (second row)
 // Row3: Notes0x10-0x17 (third row)
 ...
 private void SyncChannelStateFromSession(ChannelViewModel channel)
{
    ...
    {
        // Best-effort LED cleanup on exit
        TurnOffAllLeds();
    }
}
